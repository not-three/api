...
import { FileGetResponse } from "src/types/api/FileGetResponse";
import { FileListResponse } from "src/types/api/FileListResponse";
import { FileUploadPartResponse } from "src/types/api/FileUploadPartResponse";
...
function DefaultDecorator(notFound = true, upload = false, pass = true) {
  return applyDecorators(
    GlobalDecorator(!pass),
...
export class FilesController {
  ...
  @ApiQuery({ name: "part", type: Number, required: false })
  @ApiResponse({
    status: HttpStatus.OK,
    type: FileUploadPartResponse,
  })
  @ErrorDecorator(
  ...
  async getUploadUrl(
    ...
    @Query("part") part: string | number,
    @Req() req: Request,
  ): Promise<FileUploadPartResponse> {
    if (!length)
      throw new HttpException(
    ...
    const totalPart = file.part + 1;
    if (totalPart * 5 > this.cfg.get().fileTransfer.maxSizeInMB) {
    ...
    if (part && isNaN(Number(part)))
      throw new HttpException(
        "The part query parameter must be a number",
        HttpStatus.BAD_REQUEST,
      );
    if (part && Number(part) <= 0)
      throw new HttpException(
  ...
  async downloadFile(
    ...
    if ((json || "").toLowerCase() !== "true") res.redirect(302, url);
      ...
      res.json({
        url,
        size: await this.s3.getSize(`${id}/${file.name}`),
